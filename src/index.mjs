...
const isTest = process.env.isTest; // 是否本地测试（执行 yarn test）

// 入口文件
const entryPath = isTest ? "./example/src" : "";
const fileTree = travelFile(path.resolve(process.cwd(), entryPath));

if (isTest) {
  fs.writeFileSync("./example/dist/rely.json", JSON.stringify(fileTree));
}

function travelFile(absPath, fileTree = []) {
